...
const residentIndex = await readFile(join(hostingDist, 'index.html'), 'utf8');

await writeFile(join(hostingDist, '__resident_app', 'index.html'), residentIndex);
...
for (const entry of marketingFiles) {
  await cp(join(marketingDist, entry), join(hostingDist, entry), {
    recursive: true,
    force: true,
  });
}
